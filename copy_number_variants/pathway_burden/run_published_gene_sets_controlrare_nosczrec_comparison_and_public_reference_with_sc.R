...
for (level in c("sample", "haplotype")) {
  ...
  for (comparison_name in names(comparisons)) {
    ...
    prefix <- paste(level, "comparison_and_public_reference_ctrlRare_noSCZrec_sc_26-7-17", sep = "_")
...
write.table(all_res, file.path(out_dir, "all_results_ctrlRare_noSCZrec_sc_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
write.table(sig_summary, file.path(out_dir, "summary_counts_ctrlRare_noSCZrec_sc_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, file.path(out_dir, "summary_ctrlRare_noSCZrec_sc_26-7-17.xlsx"), overwrite = TRUE)
