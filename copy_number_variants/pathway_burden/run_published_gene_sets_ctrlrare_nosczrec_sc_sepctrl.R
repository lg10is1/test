...
out_dir <- file.path(burden_root, "ctrlRareSep_sc_case_cohort_26-7-17")
...
comparisons <- list(
  `case_cohort_vs_comparison_cohort` = c("comparison_cohort"),
  `case_cohort_vs_public_reference` = c("public_reference")
)
...
logistic_deviance_result <- function(df, predictor) {
  ...
  null <- safe_glm(case_status ~ sex + global_rare_CNV_gene_count + sc, df)
  full <- safe_glm(as.formula(paste("case_status ~ sex + global_rare_CNV_gene_count + sc +", predictor)), df)
...
run_one <- function(level, comparison_name, ctrl_names, mats, sex_meta, gene_sets) {
  ...
  rare_pass <- rep(TRUE, length(scz_count))
  rare_parts <- character()
  if ("comparison_cohort" %in% ctrl_names) {
    rare_pass <- rare_pass & comparison_cohort_freq < 0.01
    rare_parts <- c(rare_parts, "comparison_cohort_frequency<0.01")
  }
  if ("public_reference" %in% ctrl_names) {
    rare_pass <- rare_pass & public_reference_freq < 0.01
    rare_parts <- c(rare_parts, "public_reference_frequency<0.01")
  }
  rare_genes <- names(scz_count)[rare_pass]
  rare_definition <- paste0(level, "_", paste(rare_parts, collapse = " AND "))
  ...
  pheno$sc <- ifelse(pheno$cohort == "comparison_cohort", 0.9, 1.0)
  ...
  for (set_name in names(gene_sets)) {
    ...
    results[[length(results) + 1]] <- data.frame(
      ...
      RareDefinition = rare_definition,
      ...
      LogisticModel_count = "case_status ~ sex + global_rare_CNV_gene_count + sc + gene_set_rare_CNV_gene_count",
      ...
      LogisticModel_binary = "case_status ~ sex + global_rare_CNV_gene_count + sc + gene_set_rare_CNV_binary",
  ...
  for (comparison_name in names(comparisons)) {
    ...
    meta <- data.frame(
      Metric = c(
        ...
        "fisher_test", "sex_covariate_source", "global_covariate", "sc_covariate",
        "important_note"
      ),
      Value = c(
        ...
        gene_set_json,
        unique(result$results$RareDefinition)[1],
        "Deviance test comparing case_status ~ sex + global_rare_CNV_gene_count + sc vs case_status ~ sex + global_rare_CNV_gene_count + sc + gene_set_rare_CNV_gene_count",
        ...
        "global_rare_CNV_gene_count = total number of rare CNV-related genes per record after comparison-specific control-rare filter",
        "sc = 1.0 for SCZ and public_reference; sc = 0.9 for comparison_cohort/comparison_site; sc has no variation in public_reference-only comparison",
        "Exploratory comparison-specific control-rare sc-covariate model: the current control dataset frequency is <1%; SCZ recurrence is not required"
      ),
      stringsAsFactors = FALSE
    )
    comp_tag <- ifelse(identical(ctrl_names, "comparison_cohort"), "vscomparison_cohort", ifelse(identical(ctrl_names, "public_reference"), "vsAllpub", "vscomparison_and_public_reference"))
    prefix <- paste(level, comp_tag, "ctrlRare_noSCZrec_sc_26-7-17", sep = "_")
...
write.table(all_res, file.path(out_dir, "all_results_ctrlRareSep_noSCZrec_sc_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
write.table(sig_summary, file.path(out_dir, "summary_counts_ctrlRareSep_noSCZrec_sc_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, file.path(out_dir, "summary_ctrlRareSep_noSCZrec_sc_26-7-17.xlsx"), overwrite = TRUE)
