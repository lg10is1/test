...
out_dir <- file.path(burden_root, "sample_comparison_and_public_reference_sexOnly_logistic_case_cohort_26-7-17")
...
comparisons <- list(filtered_cases_vs_comparison_cohort_public_reference = c("comparison_cohort", "public_reference"))
...
logistic_deviance_result <- function(df, predictor) {
  ...
  null <- safe_glm(case_status ~ sex, df)
  full <- safe_glm(as.formula(paste("case_status ~ sex +", predictor)), df)
...
run_one <- function(level, comparison_name, ctrl_names, mats, sex_meta, gene_sets) {
  ...
  for (set_name in names(gene_sets)) {
    ...
    results[[length(results) + 1]] <- data.frame(
      ...
      LogisticModel_count = "case_status ~ sex + gene_set_rare_CNV_gene_count",
      ...
      LogisticModel_binary = "case_status ~ sex + gene_set_rare_CNV_binary",
  ...
  for (comparison_name in names(comparisons)) {
    ...
    meta <- data.frame(
      ...
      Value = c(
        ...
        "Deviance test comparing case_status ~ sex vs case_status ~ sex + gene_set_rare_CNV_gene_count",
        ...
        "global_rare_CNV_gene_count retained in output only; not used as a covariate in this exploratory sex-only logistic model",
        "Exploratory sex-only model without global rare CNV burden covariate; not intended as primary cnvGSA-like model"
      ),
      stringsAsFactors = FALSE
    )
    prefix <- paste(level, comparison_name, "36geneSets_sexOnly_logistic_26-7-17", sep = "_")
...
write.table(all_res, file.path(out_dir, "all_results_36geneSets_sexOnly_logistic_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
write.table(sig_summary, file.path(out_dir, "summary_significance_counts_sexOnly_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, file.path(out_dir, "summary_36geneSets_sexOnly_logistic_26-7-17.xlsx"), overwrite = TRUE)
