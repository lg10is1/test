...
out_dir <- file.path(burden_root, "sex_globalRare_logistic_fisher_case_cohort_26-7-16")
...
comparisons <- list(
  filtered_cases_vs_comparison_cohort = c("comparison_cohort"),
  ...
  for (comparison_name in names(comparisons)) {
    ...
    meta <- data.frame(
      ...
      Value = c(
        ...
        "Liftoff-derived extra-copy protein-coding CNV burden adaptation, not exact event-level SNP-array cnvGSA replication"
      ),
      stringsAsFactors = FALSE
    )
    prefix <- paste(level, comparison_name, "36geneSets_sex_globalRare_logistic_fisher_26-7-16", sep = "_")
...
write.table(all_res, file.path(out_dir, "all_results_36geneSets_sex_globalRare_logistic_fisher_26-7-16.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
write.table(sig_summary, file.path(out_dir, "summary_significance_counts_26-7-16.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, file.path(out_dir, "summary_36geneSets_sex_globalRare_logistic_fisher_26-7-16.xlsx"), overwrite = TRUE)
