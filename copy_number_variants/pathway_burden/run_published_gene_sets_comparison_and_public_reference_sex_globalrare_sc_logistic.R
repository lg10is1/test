...
out_dir <- file.path(burden_root, "comparison_and_public_reference_sex_globalRare_sc_logistic_case_cohort_26-7-17")
...
run_one <- function(level, comparison_name, ctrl_names, mats, sex_meta, gene_sets) {
  ...
  rare_genes <- names(scz_count)[comparison_cohort_freq < 0.01 & public_reference_freq < 0.01 & scz_count >= 2]
  ...
  for (set_name in names(gene_sets)) {
    ...
    results[[length(results) + 1]] <- data.frame(
      ...
      RareDefinition = paste0(level, "_comparison_cohort_frequency<0.01 AND public_reference_frequency<0.01 AND SCZ_count>=2"),
  ...
  for (comparison_name in names(comparisons)) {
    ...
    meta <- data.frame(
      ...
      Value = c(
        ...
        gene_set_json,
        paste0(level, "_comparison_cohort_frequency<0.01 AND public_reference_frequency<0.01 AND SCZ_count>=2"),
        ...
        "sc = 1.0 for SCZ and public_reference; sc = 0.9 for comparison_cohort/comparison_site",
        "Exploratory sc-covariate model; Liftoff-derived extra-copy protein-coding CNV burden adaptation, not exact event-level SNP-array cnvGSA replication"
      ),
      stringsAsFactors = FALSE
    )
    prefix <- paste(level, comparison_name, "36geneSets_sex_globalRare_sc_logistic_26-7-17", sep = "_")
...
write.table(all_res, file.path(out_dir, "all_results_36geneSets_sex_globalRare_sc_logistic_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
write.table(sig_summary, file.path(out_dir, "summary_significance_counts_sex_globalRare_sc_26-7-17.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, file.path(out_dir, "summary_36geneSets_sex_globalRare_sc_logistic_26-7-17.xlsx"), overwrite = TRUE)
