...
out_dir <- file.path(burden_root, "CNV_PCA_SCZ_comparison_cohort_public_reference_26-7-16")
...
inputs <- list(
  sample = list(
    ...
    comparison_cohort_sc1.0 = sample_binary_path(file.path(cnv_root, "slurm_scripts_comparison_site_new")),
  ...
  haplotype = list(
    ...
    comparison_cohort_sc1.0 = file.path(cnv_root, "slurm_scripts_comparison_site_new/haplotype_CN.xlsx"),
...
plot_one <- function(scores, meta, main_title) {
  cols <- c(SCZ = "#D55E00", comparison_cohort_sc1.0 = "#0072B2", public_reference = "#009E73")
  pch_map <- c(SCZ = 16, comparison_cohort_sc1.0 = 17, public_reference = 15)
...
write.table(all_scores, file.path(out_dir, "CNV_PCA_scores_sample_haplotype_SCZ_comparison_cohort_public_reference_26-7-16.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
write.table(all_meta, file.path(out_dir, "CNV_PCA_meta_sample_haplotype_SCZ_comparison_cohort_public_reference_26-7-16.tsv"), sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")

pdf_file <- file.path(out_dir, "CNV_PCA_SCZ_comparison_cohort_public_reference_sample_and_haplotype_26-7-16.pdf")
png_file <- file.path(out_dir, "CNV_PCA_SCZ_comparison_cohort_public_reference_sample_and_haplotype_26-7-16.png")
...
mtext("CNV PCA across SCZ, comparison_cohort sc1.0 and public_reference", outer = TRUE, cex = 1.1, font = 2)
dev.off()
